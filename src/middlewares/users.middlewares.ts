...
// tạo ra 1 req, và bỏ vào trong đó email và pwd
// nhét vào email,pwd vào trong req.body
...
// expressjs cung cấp những interface này dùng để bổ nghĩa cho object
      ...
      Authorization: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            // nếu xuống đc đầy tức là access_token có rồi
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        custom: {
          options: async (value, { req }) => {
            // nếu ko tuyền lên email_verify_token thì báo lỗi
            if (!value) {
              throw new ErrorWithStatus({
            ...
            try {
              ...
              // nếu truyền lên k đúng với database thì báo lỗi
  ...
  checkSchema(
    {
      forgot_password_token: {
        ...
        custom: {
          options: async (value, { req }) => {
            // nếu ko tuyền lên forgot_password_token thì báo lỗi
            if (!value) {
              throw new ErrorWithStatus({
            ...
            try {
              ...
              const decoded_forgot_password_token = await verifyToken({
                token: value,
                secretOrPublicKey: (process.env.JWT_SECRET_FORGOT_PASSWORD_TOKEN = '123!@#3' as string)
              })
        ...
        custom: {
          options: async (value, { req }) => {
            if (REGEX_USERNAME.test(value) === false) {
              throw Error(USERS_MESSAGES.USERNAME_MUST_BE_A_STRING)
            }
